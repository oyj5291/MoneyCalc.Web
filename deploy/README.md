...
게시 결과를 서버로 전송합니다. 아래의 IP와 SSH 키 경로는 실제 값으로 바꿉니다.
...
Ubuntu 22.04에서 `aspnetcore-runtime-10.0`을 찾지 못하면 먼저 backports 저장소를
추가한 뒤 다시 설치합니다.
...
새 게시 파일을 `/tmp/moneycalc`로 전송한 뒤 다음 순서로 교체합니다.
...
배포 실패 시 먼저 `systemctl status moneycalc`와 `journalctl -u moneycalc`를
확인합니다.
